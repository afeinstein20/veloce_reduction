...
#     yq2_norm = (YY_q2.flatten() / ((len(yq2)-1)/2.)) - 1.
    ...
    if save_medimg:
        ...
        path = bias_list[0][0:-len(dum[-1])]
        # write median bias image to file
        pyfits.writeto(path+'median_bias.fits', np.float32(medimg), overwrite=True)
...
def make_ronmask(rons, nx, ny, nq=4, gain=None, savefile=False, path=None, timit=False):
    ...
    if savefile:
        ...
        if path is None:
            ...
            pyfits.writeto(path + date + '_read_noise_mask.fits', np.float32(ronmask), overwrite=True)
...
def make_offmask_and_ronmask(offsets, rons, nx, ny, gain=None, savefiles=False, path=None, timit=False):
    ...
    if savefiles:
        ...
        if path is None:
            ...
            pyfits.writeto(path+'offmask.fits', np.float32(offmask), overwrite=True)
            ...
            pyfits.writeto(path+'read_noise_mask.fits', np.float32(ronmask), overwrite=True)
...
#             h = pyfits.getheader(path + date + '_master_bias.fits')
            ...
            pyfits.writeto(outfn, np.float32(MD), h, overwrite=True)
        else:
...
#                 h = pyfits.getheader(path + date + '_master_bias.fits')
                ...
                h['UNITS'] = 'ELECTRONS'
                pyfits.writeto(outfn, np.float32(submd), h, overwrite=True)
...
#         pyfits.append(outfn, np.float32(err_dc_bc_img), h_err, overwrite=True)
...
def bias_subtraction(imglist, MB, noneg=True, savefile=True):
    ...
    for file in imglist:
        ...
        if savefile:
            ...
            pyfits.writeto(path+'bc_'+dum[-1], np.float32(mod_img), h, overwrite=True)
    return
...
def dark_subtraction(imglist, MD, noneg=True, savefile=True):
    ...
    for file in imglist:
        ...
        if savefile:
            ...
            path = file[:-len(dum[-1])]
            h = pyfits.getheader(file)
            pyfits.writeto(path+'dc_'+dum[-1], np.float32(mod_img), h, overwrite=True)
    return
